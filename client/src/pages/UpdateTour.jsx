...
export default function UpdateTour() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      setError(false);
      const res = await fetch(`${url}/api/tour/update/${params.tourId}`, {
        method: "POST",
        headers: {
  ...
  return (
    <main className="p-3 max-w-4xl mx-auto ">
      ...
      >
        ...
        <div className="flex flex-col flex-1 gap-4 w-full md:w-2/4">
          ...
          <div className="flex flex-row sm:flex-col md:flex-row flex-wrap gap-2 justify-between">
            {formData.imageUrls.length > 0 &&
              formData.imageUrls.map((url, index) => (
                <div
                  key={url}
                  className="flex flex-col rounded-md gap-4 p-3 border items-center"
                >
                  <img
                    src={url}
                    alt="listing image"
                    className="w-40 aspect-[16/9] object-contain rounded-lg"
                  />
                  <button
                    type="button"
